...
export const labourController = {
  ...
  async get(req: Request, res: Response, next: NextFunction) { try { res.json(await labourService.get((req.params.id as string), getShop(req).id)); } catch (e) { next(e); } },
  ...
  async update(req: Request, res: Response, next: NextFunction) { try { res.json(await labourService.update((req.params.id as string), getShop(req).id, req.body)); } catch (e) { next(e); } },
  async remove(req: Request, res: Response, next: NextFunction) { try { await labourService.remove((req.params.id as string), getShop(req).id); res.sendStatus(204); } catch (e) { next(e); } },
  async salary(req: Request, res: Response, next: NextFunction) {
    try {
      const month = (req.query.month as string) || new Date().toISOString().slice(0, 7);
      res.json(await labourService.getSalary((req.params.id as string), getShop(req).id, month));
    } catch (e) { next(e); }
  },
