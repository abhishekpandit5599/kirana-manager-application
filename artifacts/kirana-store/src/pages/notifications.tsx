  ...
  const handleMarkRead = (id: number) => {
    markRead.mutate({ id }, {
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({ queryKey: ['/api/dashboard'] }); // update dashboard counter
      }
    });
  };
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {notifications.map((notification) => (
            <Card key={notification.id} className={`shadow-sm transition-colors ${getBgColor(notification.type, notification.isRead)} ${!notification.isRead ? 'border-primary/30' : ''}`}>
